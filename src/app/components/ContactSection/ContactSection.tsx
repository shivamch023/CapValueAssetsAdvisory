...
>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <div className="relative">
                    <select
                        name="service"
                        className="field-input appearance-none pr-10 cursor-pointer text-[14px]"
                        required
                    >
        ...
        <p className="text-gray-400 text-[12px] text-center">
            By submitting this form, you agree to our privacy policy. All inquiries are treated with strict confidentiality.
        </p>
    </form>
</motion.div>
...
/* INPUT FIELD */
// If your Field component is like this now:
function field({ label, type }: { label: string; type?: string }) {
    return (
        <div>
            <label className="field-label">{label}</label>
            <input type={type || "text"} className="field-input" />
        </div>
    );
}

// Update it to include name:
function Field({ label, name, type = "text" }: { label: string; name: string; type?: string }) {
    return (
        <div>
            <label className="field-label">{label}</label>
            <input type={type} name={name} className="field-input" required={label.includes("*")} />
        </div>
    );
}
